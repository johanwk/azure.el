...
(defun azure-devops-outline--display (items edges)
  ...
  (let ((content (azure-devops-outline--render items edges))
    ...
    ;; restoration does not immediately undo `pop-to-buffer'.  The buffer may
    ;; have been killed before the timer runs (for example, by a caller or test).
    (run-at-time 0 nil
                 (lambda (buffer)
...
(defun azure-devops-outline--offer-initialization ()
  "Offer to initialize Azure and return its initialization promise.
Signal a `user-error' when the user declines."
  (if (yes-or-no-p
       "Azure is not initialized; work items cannot be retrieved.  Run `azure-init' now? ")
      (azure-init)
    (user-error "Cannot retrieve Azure work items without initialization")))
...
(async-defun azure-devops-work-item-outline ()
  ...
  (unless (azure--valid-p)
    (await (azure-devops-outline--offer-initialization)))
  ;; Initialization can be aborted before all three values have been selected.
  (unless (azure--valid-p)
    (user-error "Azure initialization did not set organization, project, and team"))
