...
(defun azure-devops--build-filter-object ()
  ...
  (let ((filter-object (make-hash-table :test 'equal)))
    ...
    (when azure--area
      (puthash "System.AreaPath" azure--area filter-object))
    (when azure--iteration
      (puthash "System.IterationPath" azure--iteration filter-object))
    (when azure--tags
      (puthash "System.Tags" azure--tags filter-object))
    filter-object))
...
(defun azure-devops--select-filter-values (prompt values setter)
  "Select from VALUES using PROMPT, call SETTER, and refresh the search."
  (let* ((crm-separator "[ \t]*;[ \t]*")
         (selected (completing-read-multiple prompt values nil t)))
    (funcall setter selected)
    (azure-devops--search)))

(defun azure-devops--get-available-types ()
  "Fetch work item types, then allow the user to select multiple values."
  (interactive)
  (azure-devops--fetch-work-item-types
   (lambda (types)
     (setq azure--available-types types)
     (azure-devops--select-filter-values
      "Select work item types (separate multiple with ;): " types
      (lambda (selected) (setq azure--types selected))))))

(defun azure-devops--parse-states (data)
  "Return sorted unique state names from Azure state DATA."
  (sort (delete-dups
         (delq nil
               (mapcar (lambda (state) (cdr (assoc 'name state)))
                       (append data nil))))
        #'string-lessp))

(defun azure-devops--fetch-states (callback)
  "Fetch states for every project work-item type and call CALLBACK once."
  (azure-devops--fetch-work-item-types
   (lambda (types)
     (if (null types)
         (funcall callback nil)
       (let ((remaining (length types))
             states)
         (dolist (type types)
           (azure-get
            (format "https://dev.azure.com/{organization}/{project}/_apis/wit/workitemtypes/%s/states"
                    (url-hexify-string type))
            (cl-function
             (lambda (&key data &allow-other-keys)
               (setq states
                     (append (azure-devops--parse-states
                              (cdr (assoc 'value data)))
                             states))
               (when (= 0 (cl-decf remaining))
                 (funcall callback
                          (sort (delete-dups states) #'string-lessp)))))
            '(("api-version" . "7.1")))))))))

(defun azure-devops--get-available-states ()
  "Fetch available states, then allow the user to select multiple values."
  (interactive)
  (azure-devops--fetch-states
   (lambda (states)
     (azure-devops--select-filter-values
      "Select states (separate multiple with ;): " states
      (lambda (selected) (setq azure--state selected))))))

(defun azure-devops--classification-paths (node &optional parent)
  "Return NODE and descendant classification paths below PARENT.
Paths are built from node names because API `path' values include the
classification container (Area or Iteration), unlike work-item field values."
  (let* ((name (cdr (assoc 'name node)))
         (current (cond ((and parent name) (concat parent "\\" name))
                        (name name)
                        (t parent)))
         (children (append (cdr (assoc 'children node)) nil)))
...
(defun azure-devops--fetch-classification-paths (kind callback)
  "Fetch classification nodes of KIND and pass their paths to CALLBACK."
  (azure-get
   (format "https://dev.azure.com/{organization}/{project}/_apis/wit/classificationnodes/%s"
           kind)
   (cl-function
    (lambda (&key data &allow-other-keys)
      (funcall callback (azure-devops--classification-paths data))))
   '(("$depth" . 20) ("api-version" . "7.1"))))

(defun azure-devops--get-available-areas ()
  "Fetch area paths, then allow the user to select multiple values."
  (interactive)
  (azure-devops--fetch-classification-paths
   "Areas"
   (lambda (areas)
     (azure-devops--select-filter-values
      "Select areas (separate multiple with ;): " areas
      (lambda (selected) (setq azure--area selected))))))

(defun azure-devops--get-available-iterations ()
  "Fetch iteration paths, then allow the user to select multiple values."
  (interactive)
  (azure-devops--fetch-classification-paths
   "Iterations"
   (lambda (iterations)
     (azure-devops--select-filter-values
      "Select iterations (separate multiple with ;): " iterations
      (lambda (selected) (setq azure--iteration selected))))))

(defun azure-devops--parse-tags (data)
  "Return sorted tag names from Azure tag DATA."
  (sort (delq nil
              (mapcar (lambda (tag) (cdr (assoc 'name tag)))
                      (append data nil)))
        #'string-lessp))

(defun azure-devops--fetch-tags (callback)
  "Fetch project tags and pass them to CALLBACK."
  (azure-get "https://dev.azure.com/{organization}/{project}/_apis/wit/tags"
             (cl-function
              (lambda (&key data &allow-other-keys)
                (funcall callback
                         (azure-devops--parse-tags
                          (cdr (assoc 'value data))))))
             '(("api-version" . "7.1-preview.1"))))

(defun azure-devops--get-available-tags ()
  "Fetch available tags, then allow the user to select multiple values."
  (interactive)
  (azure-devops--fetch-tags
   (lambda (tags)
     (azure-devops--select-filter-values
      "Select tags (separate multiple with ;): " tags
      (lambda (selected) (setq azure--tags selected))))))
...
(defun azure-devops--clear-unsupported-filter (field)
  "Clear the selected search filter corresponding to Azure FIELD.
Return non-nil when FIELD is a filter managed by this package."
  (pcase (downcase field)
    ("system.workitemtype" (and azure--types
                                (progn (setq azure--types nil) t)))
    ("system.assignedto" (and azure--assignees
                              (progn (setq azure--assignees nil) t)))
    ("system.state" (and azure--state
                         (progn (setq azure--state nil) t)))
    ("system.areapath" (and azure--area
                            (progn (setq azure--area nil) t)))
    ("system.iterationpath" (and azure--iteration
                                 (progn (setq azure--iteration nil) t)))
    ("system.tags" (and azure--tags
                        (progn (setq azure--tags nil) t)))
    (_ nil)))

(defun azure-devops--search-error (&rest args)
  "Handle an Azure work-item search failure from request callback ARGS.
Clear and return its field name for an unavailable search filter.  Signal all
other failures normally."
  (let* ((data (plist-get args :data))
         (response (plist-get args :response))
         (error-thrown (plist-get args :error-thrown))
         (status (or (ignore-errors
                       (request-response-status-code response))
                     (and (equal (car-safe error-thrown) 'error)
                          (equal (cadr error-thrown) 'http)
                          (nth 2 error-thrown))))
         (server-message (cdr (assoc 'message data)))
         (field (and (= (or status 0) 400)
                     server-message
                     (string-match "[Uu]nknown filter \\[\\([^]]+\\)\\]"
                                   server-message)
                     (match-string 1 server-message))))
    (if (and field (azure-devops--clear-unsupported-filter field))
        field
      (error "Azure work-item search failed%s"
             (if server-message
                 (format ": %s" server-message)
  ...
  (let ((url "https://almsearch.dev.azure.com/{organization}/{project}/_apis/search/workitemsearchresults")
    ...
    (azure-post url
		  ...
		  ;; `azure-req' JSON-encodes the whole request once.
                ...
                nil
                (lambda (&rest args)
                  ;; Retry after removing an unsupported filter.  Azure may
                  ;; reject another selected filter on the next request.
                  (let ((field (apply #'azure-devops--search-error args)))
                    (message "Azure Search filter %s is not available for this project; it was cleared"
                             field)
                    (azure-devops--search text skip))))))
...
(defun azure-devops--select-work-item (results &optional default-id prompt)
  "Prompt for and return one work item from RESULTS.

Each element of RESULTS has the form (ID TITLE STATE ASSIGNEE).
DEFAULT-ID, when present in RESULTS, is the preselected candidate.  Use
PROMPT instead of the standard work-item prompt when it is non-nil."
  (unless results
         ...
         (default-item
          (and default-id
               (seq-find (lambda (item)
                           (equal (car item) (format "%s" default-id)))
                         results)))
         (default (and default-item
                       (azure-devops--work-item-candidate default-item)))
         (completion-extra-properties
          '(:category azure-devops-work-item))
         (choice (completing-read
                  (or prompt "Work item (state, assignee, title): ")
...
(defun azure-devops--create-parent-id ()
  "Return the work-item ID at the current Org context, or nil.

In an Azure work-items outline, use AZURE_ID at point.  In an individual
work-item buffer, use the document's id property."
  (when (derived-mode-p 'org-mode)
    (let ((id
           (if (string-prefix-p "*Azure work items" (buffer-name))
               (save-excursion
                 (when (org-before-first-heading-p)
                   (user-error "Move point to a work-item heading first"))
                 (org-back-to-heading t)
                 (org-entry-get nil "AZURE_ID" nil))
             (azure-devops--document-property "id"))))
      (when (and id (string-match-p "\\`[0-9]+\\'" id))
...
(defun azure-devops--prompt-work-item-create (types &optional default-parent-id)
  "Prompt for type, title, and parent, then create a work item.

Choose the type from TYPES.  DEFAULT-PARENT-ID is preselected in the parent
completion list when that work item is in the returned Azure search results."
  (unless types
    (user-error "This Azure DevOps project has no available work-item types"))
  (let* ((item-type (completing-read "Item type: " types nil t))
         (title (read-from-minibuffer "Item title: "))
         (parent
          (azure-devops--select-work-item
           (azure-devops--wait-for-link-search "")
           default-parent-id
           "Parent (state, assignee, title): ")))
    (azure-devops-work-item-create
     item-type title (string-to-number (car parent)))))

(defun azure-devops-work-item-create (&optional item-type title parent-id)
  "Create a new work item of ITEM-TYPE with TITLE and PARENT-ID.

Interactively, retrieve the current project's work-item types and work items
from Azure DevOps before prompting.  The current work-item document id, or the
...
See the Azure DevOps documentation for the work-items Create API."
  ...
  (if (eq item-type :interactive)
      (if parent-id
          (let ((default-parent-id parent-id))
            (azure-devops--fetch-work-item-types
             (lambda (types)
               (azure-devops--prompt-work-item-create
                types default-parent-id))))
        (azure-devops--fetch-work-item-types
         #'azure-devops--prompt-work-item-create))
  ...
  (promise-new
   (lambda (resolve _reject)
     (azure-get
      ...
      (cl-function
       (lambda (&rest args &key response error-thrown &allow-other-keys)
         ;; Do not guard the accessor with `request-response-p'.  That predicate
         ;; may be inlined when this file is byte-compiled, which prevents tests
         ;; (and compatible response implementations) from overriding it.
         (let ((status (or (and response
                                (ignore-errors
                                  (request-response-status-code response)))
                           ;; Compiled struct accessors cannot reliably be
                           ;; overridden in tests; `request' also includes the
                           ;; HTTP status in ERROR-THROWN.
                           (and (listp error-thrown)
                                (memq 404 error-thrown)
                                404))))
